...
::: {.notes}
Timing: 1:00 to 2:00. Connect to the earlier reproducibility lesson and give participants time to read the comic. Ask whether automation removes work or shifts it towards specifying, checking, and maintaining code. Reveal the question, take one response, then reveal the need for feedback.
...
## Agentic programming needs defensive code

::: {.blockquote}
This session is not about how to operate AI agents. It is about writing code that agents can navigate and that humans or agents can validate.
:::
...
- focused tests for expected behaviour
:::

::::

::: {.fragment .text-center}
🤖 **Agent:** Done. &nbsp;&nbsp; 🧑‍🔬 **You:** Show me the tests and the diff.
:::

::: {.notes}
Timing: 2:00 to 3:00. Use this slide to establish why the lesson focuses on functions, assertions, and tests before presenting the learning outcomes. Read the checklist, then reveal the short exchange. AI agents can make changes quickly, but the researcher remains responsible for the function's assumptions, the data meaning, and every changed result.
:::
...
<img src="Materials/Illustrations/functional_revelation.png"
     ...
     style="width:100%; height:465px; object-fit:cover; object-position:68% center; border-radius:18px;">
...
## Extra: compact workflow

<br>

::: {.code-scroll style="max-height: 430px; overflow-y: auto; scrollbar-gutter: stable;"}
...
::: {.notes}
Timing: 56:00 to 58:00. Present this as an optional compact version of the workflow participants have already seen. Trace one species data frame into a local model and then a plot. `map2()` collects the three plots before `plot_grid()` combines them. The separated version remains easier to debug and review.
:::
...
::: {.notes}
Timing: 87:00 to 89:00. Move through red, green, another revealing test, and refactor. Explain that a red test can describe missing behaviour rather than an accidental regression. Present TDD as one useful workflow, not a requirement for every exploratory line of code.
:::
